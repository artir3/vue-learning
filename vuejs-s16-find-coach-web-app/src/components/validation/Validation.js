...
const emailVal = email => EMAIL_REGEX_RFC_2822.test(email);
const textVal = (text, minLength = 0) => text.length > minLength;
const numberVal = number => number & (number > 0);
const checkboxVal = checkboxArray => checkboxArray.length > 0;

export { emailVal, textVal, numberVal, checkboxVal };
